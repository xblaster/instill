 ...
 */
export function convertGitHubUrlToRawUrl(repoUrl, filePath, branch = DEFAULT_BRANCH) {
 ...
 */
export async function fetchSkillFromRemote(source, skillName, filePath = `skills/${skillName}.md`) {
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                throw new Error(`Skill "${skillName}" not found in repository "${source.name}" (${source.url})`);
            }
            throw new Error(`Failed to fetch skill from ${source.name}: HTTP ${response.status} ${response.statusText}`);
        }
        ...
        throw new Error(`Error fetching skill "${skillName}" from "${source.name}": ${error instanceof Error ? error.message : String(error)}`);
    }
}
