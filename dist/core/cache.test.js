...
import { getCachePath, ensureCacheDir, cacheSkill, getCachedSkill, isCacheValid, clearCache, getCacheMetadata, } from './cache.js';
...
describe('cache module', () => {
    ...
    describe('getCachedSkill', () => {
        it('returns cached skill content', async () => {
            ...
            expect(vi.mocked(commandr.readFile)).toHaveBeenCalledWith('.instill/.cache/skills/test-skill.md');
        });
    ...
    describe('clearCache', () => {
        it('clears all cache when no skill specified', async () => {
            ...
            expect(vi.mocked(commandr.writeFile)).toHaveBeenCalledWith('.instill/.cache/metadata.json', JSON.stringify({}, null, 2));
        });
        it('clears specific skill cache', async () => {
            ...
            expect(vi.mocked(commandr.deleteFile)).toHaveBeenCalledWith('.instill/.cache/skills/test-skill.md');
        });
    });
