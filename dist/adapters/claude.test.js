...
describe('ClaudeAdapter', () => {
    ...
    it('should inject skills into CLAUDE.md with markers', async () => {
        ...
        vi.mocked(commandr.readFile).mockImplementation(async (path) => {
            if (path.includes('skill1.md'))
                return 'Skill 1 Logic';
            if (path === 'CLAUDE.md')
                return 'User content here';
            throw new Error('Not found');
        });
        await adapter.installSkills(['skill1']);
        expect(commandr.writeFile).toHaveBeenCalledWith('CLAUDE.md', expect.stringContaining('<!-- INSTILL_START -->'));
        expect(commandr.writeFile).toHaveBeenCalledWith('CLAUDE.md', expect.stringContaining('Skill 1 Logic'));
    });
